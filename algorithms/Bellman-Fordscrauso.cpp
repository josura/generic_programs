...
template <class T> class LGraph {
	...
	int Bellman_Ford(T s, T dest,int k){
...
int main(){
	...
	while(!read.eof()){
		read>>n>>m>>k;
			...
			int mindist=grafo.Bellman_Ford(sorgente, destinazione,k);
			if(mindist==inf) write << "inf.";
			//else if(mindist==negcycle) write << "undef.";
			else write << mindist;
			cout<<"sono fuori";
			write << endl;
			cout<<"questo non lo supero";
	}
	read.close();
	write.close();
	return 0;
