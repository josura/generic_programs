...
class Mgraph{
	...
	public:
		...
		void printstartend(){
			DFS();
			for(int i=0; i<numver; i++){
				cout << "("<<*K[i]<<","<<discovered[i]<<","<<finished[i];
				cout<<") ";	
				}
			
		}
		void printstartend(ofstream& stream){
			DFS();
			for(int i=0; i<numver; i++){
				stream << "("<<*K[i]<<","<<discovered[i]<<","<<finished[i];
				stream<<") ";	
				}
			stream<<endl;
		}
